  ...
  if (!isOpen && !sidebarVisible) {
    ...
    return (
      <button
        ...
        aria-label={t('books:openBooksSidebar')}
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
        </svg>
        <span className="hidden sm:inline text-xs">{t('books:books')}</span>
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          {isViewerMode ? (
            ...
            <h2 className="text-lg font-semibold text-foreground">{t('books:campaignBooks')}</h2>
          )}
          <div className="flex items-center gap-1 shrink-0">
            ...
            {isViewerMode && internalBookId && (
              <button
                ...
                aria-label={t('books:backToBookList')}
                title={t('books:bookList')}
              >
        ...
        {isViewerMode ? (
          ...
          <div className="flex-1 relative bg-[#525659]">
            ...
            {iframeLoading && (
              <div className="absolute inset-0 flex items-center justify-center z-10 bg-[#525659]">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-xs text-white/60">{t('books:loadingPdf')}</span>
                </div>
              </div>
            )}
            ...
            {iframeError && !iframeLoading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center">
                ...
                <p className="text-sm text-danger font-medium mb-1">{t('books:failedToLoadPdf')}</p>
                <p className="text-xs text-muted-foreground">
                  {t('books:pdfLoadErrorDetail')}
                </p>
              </div>
            ) : null}
          ...
          <>
              ...
              {!loadingList && books.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {isGM ? t('books:emptyListGm') : t('books:emptyListPlayer')}
                  </p>
                </div>
              )}
                  ...
                  {books.map((book) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        aria-label={t('books:openBook', { name: book.name })}
                        title={t('common:open')}
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="px-4 py-2.5 border-t border-border shrink-0 text-[11px] text-muted-foreground">
              {t('books:bookCount', { count: books.length })}
            </div>
          </>
        )}
      </aside>
      ...
      {minimized && (
        <button
          ...
          className="fixed top-24 right-0 z-40 flex items-center gap-2 px-3 py-2 rounded-l-lg bg-surface border border-r-0 border-border text-sm font-medium text-foreground hover:bg-hover transition-colors shadow-lg"
          aria-label={t('books:restoreBooksSidebar')}
          title={t('books:restoreBooksSidebar')}
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
          </svg>
          <span className="hidden sm:inline text-xs">{isViewerMode ? activeBookName : t('books:books')}</span>
        </button>
      )}
    </>
  )
