...
function LoginForm() {
  ...
  const [error, setError] = useState<string | null>(() => {
    if (errorParam) {
      ...
      return errors[errorParam] ?? 'Authentication failed'
    }
    return null
  })
  const [submitting, setSubmitting] = useState(false)

  // If coming from an invitation, save it before OAuth redirect
  const handleOAuthClick = (provider: string) => {
    // Check if we have an invitation redirect
    ...
    // The anchor href will handle navigation
  }
    ...
    <div className="w-full max-w-sm space-y-6">
      ...
      <form onSubmit={handleSubmit} className="card !p-6 space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <a
            href={`${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3000/api'}/auth/discord`}
            className="btn-ghost flex-1"
