  ...
  return <form onSubmit={props.onCreateTemplate} className="rounded-lg border border-primary/20 bg-background/50 p-4 space-y-3">
    ...
    {activeTab==='characterSections'&&<div>
      <div className="space-y-2 mt-1">
        {(props.newCharacterSections||[]).map((name, idx) => (
          <div key={idx} className="flex items-center gap-1.5">
            <input className="input-field flex-1" value={name} onChange={e => props.onUpdateNewCharacterSection?.(idx, e.target.value)} placeholder="Section name (e.g. Talents)" />
      ...
      <div className="rounded-lg border border-border bg-background/30 p-3 space-y-2">
        ...
        {(props.newAcEnabled??false)&&<div className="space-y-2 pl-2">
          ...
          <div><label className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 block">Attribute Modifiers</label>
            <div className="space-y-1">
              {(props.newTemplateAttrs2||props.newTemplateAttrs||[]).filter(a=>a.key.trim()&&a.name.trim()).map(attr=>( <label key={attr.key} className="flex items-center gap-2 text-xs text-foreground cursor-pointer py-1"><input type="checkbox" className="w-3 h-3 rounded accent-primary" checked={acAttributeIds.includes(attr.key.trim())} onChange={()=>props.onToggleNewAcAttributeId?.(attr.key.trim())}/><span>{attr.name.trim()} Modifier</span></label> ))}
            </div>
          </div>
        </div>}
      </div>
    ...
    {editTab==='ac'&&props.onAddEditAcField&&<div><div className="space-y-2 mt-1">
      <div className="rounded-lg border border-border bg-background/30 p-3 space-y-2">
        ...
        {(props.editAcEnabled??false)&&<div className="space-y-2 pl-2">
          ...
          <div><label className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 block">Attribute Modifiers</label>
            <div className="space-y-1">{allEditAttrsForAc.filter(a=>a.key.trim()&&a.name.trim()).map(attr=>(<label key={attr.key} className="flex items-center gap-2 text-xs text-foreground cursor-pointer py-1"><input type="checkbox" className="w-3 h-3 rounded accent-primary" checked={editAcAttributeIdsValues.includes(attr.key.trim())} onChange={()=>props.onToggleEditAcAttributeId?.(attr.key.trim())}/><span>{attr.name.trim()} Modifier</span></label>))}</div>
          </div>
        </div>}
      </div>
    </div></div>}
