...
interface Template {
  ...
  templateSkills?: { id: string; name: string; description: string | null }[]
...
export default function AdventureDetailPage() {
  ...
  const [newTemplateSkills, setNewTemplateSkills] = useState<{ name: string; description: string }[]>([]); const [editTemplateSkills, setEditTemplateSkills] = useState<{ name: string; description: string }[]>([]); const [templateSaving, setTemplateSaving] = useState(false)
  ...
  function addNewSkillRow() { setNewTemplateSkills(p => [...p, { name: '', description: '' }]) }; function removeNewSkillRow(i: number) { setNewTemplateSkills(p => p.filter((_,j)=>j!==i)) }; function updateNewSkill(i: number, f: 'name'|'description', v: string) { setNewTemplateSkills(p => p.map((s,j)=>j===i?{...s,[f]:v}:s)) }
  function addEditSkillRow() { setEditTemplateSkills(p => [...p, { name: '', description: '' }]) }; function removeEditSkillRow(i: number) { setEditTemplateSkills(p => p.filter((_,j)=>j!==i)) }; function updateEditSkill(i: number, f: 'name'|'description', v: string) { setEditTemplateSkills(p => p.map((s,j)=>j===i?{...s,[f]:v}:s)) }
  ...
  async function handleCreateTemplate(e: FormEvent) { e.preventDefault(); setTemplateError(null)
    ...
    try {
      await api.post(`/adventures/${id}/templates`, {
        ...
        skills: newTemplateSkills.filter(s=>s.name.trim()).map(s=>({name:s.name.trim(),description:s.description.trim()||undefined})),
  ...
  function startEditTemplate(t: Template) { setEditingTemplateId(t.id); setEditTemplateName(t.name); setEditTemplateDescription(t.description??'');
    ...
    setEditTemplateSkills((t.templateSkills||[]).map(s=>({name:s.name,description:s.description??''})));
  ...
  async function handleUpdateTemplate(e: FormEvent) { e.preventDefault(); if(!editingTemplateId)return; setEditingTemplateError(null)
    ...
    try {
      await api.patch(`/adventures/${id}/templates/${editingTemplateId}`, {
        ...
        skills: editTemplateSkills.filter(s=>s.name.trim()).map(s=>({name:s.name.trim(),description:s.description.trim()||undefined})),
...
function TemplatesSection(props: {
  ...
  newTemplateSkills?: { name: string; description: string }[]; editTemplateSkills?: { name: string; description: string }[]
  onAddSkill?: () => void; onRemoveSkill?: (i: number) => void; onUpdateSkill?: (i: number, f: 'name'|'description', v: string) => void
  onAddEditSkill?: () => void; onRemoveEditSkill?: (i: number) => void; onUpdateEditSkill?: (i: number, f: 'name'|'description', v: string) => void
...
function NewTemplateForm(props: {
  ...
  newAttrModifierFormula: string; newSkillFormula: string; newTemplateSkills?: { name: string; description: string }[]; newTemplateProfiles?: { name: string; options: { label: string; value: number }[] }[]; newTemplateFields?: { key: string; label: string }[]
  ...
  return <form onSubmit={props.onCreateTemplate} className="rounded-lg border border-primary/20 bg-background/50 p-4 space-y-3">
    ...
    {activeTab==='skills'&&<div>
      ...
      <div className="space-y-2 mt-1">{(props.newTemplateSkills||[]).map((s,idx)=><CollapsibleSkillCard key={idx} index={idx} skill={s} onUpdateSkill={props.onUpdateSkill} onRemove={()=>props.onRemoveSkill?.(idx)}/>)}</div><button type="button" onClick={props.onAddSkill} className="btn-ghost text-xs mt-2">+ Add Skill</button></div>}
...
function TemplateRow(props: {
  template: Template; isGM: boolean; isEditing: boolean; editName: string; editDescription: string; editAttrs: { key: string; name: string }[]; editAttrModifierFormula: string; editSkillFormula: string; editFields?: { key: string; label: string }[]; editSkills?: { name: string; description: string }[]; editError: string | null; saving: boolean
  ...
  if(props.isEditing) return <form onSubmit={props.onUpdate} className="rounded-lg border border-primary/30 bg-background/50 p-4 space-y-3">
    ...
    {editTab==='skills'&&<div>
      ...
      <div className="space-y-2 mt-1">{(props.editSkills||[]).map((s,idx)=><CollapsibleSkillCard key={idx} index={idx} skill={s} onUpdateSkill={props.onUpdateSkill} onRemove={()=>props.onRemoveSkill?.(idx)}/>)}</div><button type="button" onClick={props.onAddSkill} className="btn-ghost text-xs mt-2">+ Add Skill</button></div>}
