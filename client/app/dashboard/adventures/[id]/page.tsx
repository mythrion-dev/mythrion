...
export default function AdventureDetailPage() {
  ...
  // Point Pool helpers
  function addNewPool() { setNewTemplatePools(p => [...p, { name: '', slug: '', description: '', defaultMaximum: 0, currentStartsFull: true, editableByPlayer: true }]) }
  ...
  function updateNewPoolNumber(i: number, f: 'defaultMaximum', v: number) { setNewTemplatePools(p => p.map((m,j) => j===i ? {...m,[f]:v} : m)) }
  function updateNewPoolBool(i: number, f: 'currentStartsFull'|'editableByPlayer', v: boolean) { setNewTemplatePools(p => p.map((m,j) => j===i ? {...m,[f]:v} : m)) }
  function addEditPool() { setEditTemplatePools(p => [...p, { name: '', slug: '', description: '', defaultMaximum: 0, currentStartsFull: true, editableByPlayer: true }]) }
  ...
  function updateEditPoolNumber(i: number, f: 'defaultMaximum', v: number) { setEditTemplatePools(p => p.map((m,j) => j===i ? {...m,[f]:v} : m)) }
  function updateEditPoolBool(i: number, f: 'currentStartsFull'|'editableByPlayer', v: boolean) { setEditTemplatePools(p => p.map((m,j) => j===i ? {...m,[f]:v} : m)) }
  ...
  async function handleCreateTemplate(e: FormEvent) { e.preventDefault(); setTemplateError(null)
    ...
    try {
      await api.post(`/adventures/${id}/templates`, {
        ...
        pointPools: newTemplatePools.filter(p=>p.name.trim()&&p.slug.trim()).map(p=>({name:p.name.trim(),slug:p.slug.trim(),description:p.description.trim()||undefined,defaultMaximum:p.defaultMaximum,currentStartsFull:p.currentStartsFull,editableByPlayer:p.editableByPlayer})),
  ...
  function startEditTemplate(t: Template) { setEditingTemplateId(t.id); setEditTemplateName(t.name); setEditTemplateDescription(t.description??'');
    ...
    setEditTemplatePools((t.pointPools||[]).map(p=>({name:p.name,slug:p.slug,description:p.description??'',defaultMaximum:p.defaultMaximum,currentStartsFull:p.currentStartsFull,editableByPlayer:p.editableByPlayer})));
  ...
  async function handleUpdateTemplate(e: FormEvent) { e.preventDefault(); if(!editingTemplateId)return; setEditingTemplateError(null)
    ...
    try {
      await api.patch(`/adventures/${id}/templates/${editingTemplateId}`, {
        ...
        pointPools: editTemplatePools.filter(p=>p.name.trim()&&p.slug.trim()).map(p=>({name:p.name.trim(),slug:p.slug.trim(),description:p.description.trim()||undefined,defaultMaximum:p.defaultMaximum,currentStartsFull:p.currentStartsFull,editableByPlayer:p.editableByPlayer})),
      ...
      {!editing ? (<div className="space-y-6 mt-6">
        ...
        {activeTab==='campaign' ? (<>
          ...
          <TemplatesSection templates={templates} isGM={isGM} showNewTemplate={showNewTemplate} editingTemplateId={editingTemplateId}
            ...
            onAddPool={addNewPool} onRemovePool={removeNewPool} onUpdatePool={updateNewPool} onUpdatePoolNumber={updateNewPoolNumber} onUpdatePoolBool={updateNewPoolBool}
            onAddEditPool={addEditPool} onRemoveEditPool={removeEditPool} onUpdateEditPool={updateEditPool} onUpdateEditPoolNumber={updateEditPoolNumber} onUpdateEditPoolBool={updateEditPoolBool}
...
function TemplatesSection(props: {
  ...
  onAddPool?: () => void; onRemovePool?: (i: number) => void; onUpdatePool?: (i: number, f: 'name'|'slug'|'description', v: string) => void; onUpdatePoolNumber?: (i: number, f: 'defaultMaximum', v: number) => void; onUpdatePoolBool?: (i: number, f: 'currentStartsFull'|'editableByPlayer', v: boolean) => void
  onAddEditPool?: () => void; onRemoveEditPool?: (i: number) => void; onUpdateEditPool?: (i: number, f: 'name'|'slug'|'description', v: string) => void; onUpdateEditPoolNumber?: (i: number, f: 'defaultMaximum', v: number) => void; onUpdateEditPoolBool?: (i: number, f: 'currentStartsFull'|'editableByPlayer', v: boolean) => void
    ...
    {activeTab==='ac'&&props.onNewAcEnabledChange&&<div><div className="space-y-2 mt-1">
      <div className="rounded-lg border border-border bg-background/30 p-3 space-y-2">
        ...
        {(props.newAcEnabled??false)&&<div className="space-y-2 pl-2">
          ...
          <div><label className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 block">Attribute Modifiers</label>
            <div className="space-y-1">
              {(props.newTemplateAttrs2||props.newTemplateAttrs||[]).filter(a=>a.key.trim()&&a.name.trim()).map(attr=>(
                <label key={attr.key} className="flex items-center gap-2 text-xs text-foreground cursor-pointer py-1">
                  <input type="checkbox" className="w-3 h-3 rounded accent-primary" checked={acAttributeIds.includes(attr.key.trim())} onChange={()=>props.onToggleNewAcAttributeId?.(attr.key.trim())}/>
                  <span>{attr.name.trim()} Modifier</span>
                </label>
              ))}
              {(props.newTemplateAttrs2||props.newTemplateAttrs||[]).filter(a=>a.key.trim()&&a.name.trim()).length===0&&<p className="text-xs text-muted italic">Add attributes to the template first.</p>}
            </div>
          </div>
        </div>}
      </div>
...
function TemplateRow(props: {
  ...
  onAddProfileOption?: (pIdx: number) => void; onRemoveProfileOption?: (pIdx: number, oIdx: number) => void; onUpdateProfileOption?: (pIdx: number, oIdx: number, f: 'label'|'value', v: string|number) => void
  onUpdateProfileTargetMode?: (i: number, mode: string) => void; onToggleProfileSkill?: (i: number, skillId: string) => void
  editPools?: PointPool[]; onAddPool?: () => void; onRemovePool?: (i: number) => void; onUpdatePool?: (i: number, f: 'name'|'slug'|'description', v: string) => void; onUpdatePoolNumber?: (i: number, f: 'defaultMaximum', v: number) => void; onUpdatePoolBool?: (i: number, f: 'currentStartsFull'|'editableByPlayer', v: boolean) => void
  editAcEnabled?: boolean; editAcFields?: { name: string; key: string; defaultValue: string; editableByPlayer: boolean; description: string }[]
  editAcAttributeIds?: string[]; editTemplateAttrs2?: { key: string; name: string }[]
  onEditAcEnabledChange?: (v: boolean) => void
  onAddEditAcField?: () => void; onRemoveEditAcField?: (i: number) => void; onUpdateEditAcField?: (i: number, f: 'name'|'key'|'defaultValue'|'description', v: string) => void; onUpdateEditAcFieldEditable?: (i: number, v: boolean) => void
    ...
    {editTab==='ac'&&props.onAddEditAcField&&<div><div className="space-y-2 mt-1">
      <div className="rounded-lg border border-border bg-background/30 p-3 space-y-2">
        ...
        {(props.editAcEnabled??false)&&<div className="space-y-2 pl-2">
          <div><label className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 block">AC Components</label>
            <div className="space-y-1">
              {(props.editAcFields||[]).map((f: any,idx)=><div key={idx} className="rounded border border-border/50 bg-background/20 p-2 space-y-1">
          ...
          <div><label className="text-xs font-semibold text-muted uppercase tracking-wider mb-2 block">Attribute Modifiers</label>
            <div className="space-y-1">
              {allEditAttrsForAc.filter(a=>a.key.trim()&&a.name.trim()).map(attr=>(
                <label key={attr.key} className="flex items-center gap-2 text-xs text-foreground cursor-pointer py-1">
                  <input type="checkbox" className="w-3 h-3 rounded accent-primary" checked={editAcAttributeIdsValues.includes(attr.key.trim())} onChange={()=>props.onToggleEditAcAttributeId?.(attr.key.trim())}/>
                  <span>{attr.name.trim()} Modifier</span>
                </label>
              ))}
              {allEditAttrsForAc.filter(a=>a.key.trim()&&a.name.trim()).length===0&&<p className="text-xs text-muted italic">Add attributes to the template first.</p>}
            </div>
          </div>
        </div>}
      </div>
