...
function CollapsibleSkillRow({ skill, result, profiles, selections, active, others, onToggleActive, onOthersChange, onProfileChange, onAttributeChange, templateAttributes, expandedSkillId, onExpandToggle, modifiersEnabled }: { skill: SkillValue; result: number | null; profiles: SkillModifierProfile[]; selections: Record<string, string | null>; active: boolean; others: number; onToggleActive: () => void; onOthersChange: (v: number) => void; onProfileChange: (profileId: string, optionId: string | null) => void; onAttributeChange?: (attributeId: string | null) => void; templateAttributes?: { id: string; key: string; name: string }[]; expandedSkillId: string | null; onExpandToggle: (id: string) => void; modifiersEnabled?: boolean }) {
  ...
  const hasAttrDropdown = (skill.skill.allowedAttributeIds?.length ?? 0) > 0 && !!templateAttributes && !!onAttributeChange && modifiersEnabled !== false
                        ...
                        {(a.summonSkills ?? []).length === 0 ? (
                          ...
                          <div className="space-y-1.5">
                            {(a.summonSkills ?? []).map(ss => {
                              ...
                              const hasAttrDropdown = (ss.skill.allowedAttributeIds?.length ?? 0) > 0 && template.attributeModifiersEnabled !== false
                              return (
